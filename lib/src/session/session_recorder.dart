...
class _SessionRecorderReporter {
  ...
  Future<void> _flush() async {
    ...
    if (!_recorder.config.shouldSend) {
      if (_recorder.config.debugLog) {
        SessionLogger.mlog(
          ">> [SessionRecorder] Not send it cause `shouldSend` is [false].",
        );
      }
      return;
    }
  ...
  Future<void> _send(Chunk chunk) async {
    ...
    final uri = Uri.parse(_recorder.config.endpoint);
